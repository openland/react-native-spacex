...
class Thruster {
  ...
  constructor(configs: Array<ThrusterConfig>, protocol: String?, queue: DispatchQueue, onSuccess: (socket: DispatchedWebSocketClient) -> Unit) {
  ...
  private fun restartBucket(bucket: Int) {
    ...
    val ws = DispatchedWebSocketClient(url, this.protocol, this.queue)
