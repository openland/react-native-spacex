...
import com.reactnativespacex.impl.OperationDefinition
import com.reactnativespacex.impl.OperationKind
import com.reactnativespacex.impl.store.*
import com.reactnativespacex.impl.store.persistence.SpaceXPersistence
...
class SpaceXStore(context: Context, name: String?) {
    ...
    fun readQuery(operation: OperationDefinition, arguments: JSONObject, queue: DispatchQueue, callback: (result: QueryReadResult) -> Unit) {
        this.storeQueue.async {
            prepareRead(operation, arguments) {
                ...
                val res = readRootFromStore(ROOT_QUERY, store, operation.selector, arguments)
    ...
    fun readQueryAndWatch(operation: OperationDefinition, arguments: JSONObject, queue: DispatchQueue, callback: (result: QueryReadAndWatchResult) -> Unit) {
        this.storeQueue.async {
            prepareRead(operation, arguments) {
                ...
                // Calculate keys
                // TODO: Optimize
                val normalized = normalizeResponse(ROOT_QUERY, operation.selector, arguments, res.second!!)
    ...
    private fun prepareRead(operation: OperationDefinition, variables: JSONObject, callback: () -> Unit) {
        val missing = trace("collectMissingKeys") { collectMissingKeysRoot(ROOT_QUERY, store, operation.selector, variables) }
        if (missing.isNotEmpty()) {
